...
CREATE_MODULES_EP_COMMAND_JSON=` echo $JSON | ./rest-ep-json.bash "$MODULES_EP_PROMPT" $MODULES_EP_NAME $MODULES_EP_PORT | ./rest-ep-curl-command.bash `

DEPLOY_EP_PORT=$(($BASEPORT+2))
DEPLOY_EP_NAME="$NAME-DEPLOY"
DEPLOY_EP_PROMPT="Create $NAME Deploy EP"
CREATE_DEPLOY_EP_COMMAND_JSON=` echo $JSON | ./rest-ep-json.bash "$DEPLOY_EP_PROMPT" $DEPLOY_EP_NAME $DEPLOY_EP_PORT | ./rest-ep-curl-command.bash `
